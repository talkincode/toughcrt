...
class RadiusSession:
    ...
    iplist = itertools.cycle(ip for ip in IP('%s.0.0.0/15'%(random.randint(1,172) )))
    sessions = {}

    def __init__(self, config, dbengine,statcache=None, radius_ipaddr=None):
        self.config = config
        self.running = False
    ...
    @staticmethod
    def stop_session(ipaddr=None,session_id=None,username=None):
        if session_id:
            ...
            ids = []
            for session in RadiusSession.sessions.values():
                if session.session_data['User-Name'] == username:
                    ids.append(session.session_id)
            for sid in ids:
                session = RadiusSession.sessions.pop(sid)
                session.stop()
    ...
    def start(self, username, password, challenge=None, chap_pwd=None, **kwargs):
        ...
        if challenge and chap_pwd:
            ...
            loginfo.append(repr(auth_req))
            self.statcache.incr(statc.auth_request_cache_key)
            auth_resp = yield self.radius.send_auth(**auth_req)

        loginfo.append(message.format_packet_log(auth_resp))

        if auth_resp.code== packet.AccessReject:
            self.statcache.incr(statc.auth_reject_cache_key)
        ...
        if auth_resp.code== packet.AccessAccept:
            self.statcache.incr(statc.auth_accept_cache_key)
            ...
            self.statcache.incr(statc.acct_start_cache_key)
            ...
            if acct_resp.code == packet.AccountingResponse:
                self.statcache.incr(statc.acct_resp_cache_key)
                self.running = True
    ...
    def update(self):
        ...
        self.statcache.incr(statc.acct_update_cache_key)
        acct_resp = yield self.radius.send_acct(**self.session_data)
        self.statcache.incr(statc.acct_resp_cache_key)
    ...
    def stop(self):
        ...
        self.session_data["Acct-Input-Octets"]  +=  random.randint(1048576, 4096000)
        self.session_data['Acct-Session-Time'] = (int(time.time()) - self.session_start)
        self.statcache.incr(statc.acct_stop_cache_key)
        acct_resp = yield self.radius.send_acct(**self.session_data)
        self.statcache.incr(statc.acct_resp_cache_key)
        defer.returnValue(acct_resp)
